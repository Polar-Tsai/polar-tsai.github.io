...
同一張 cover 會同時出現在「社群分享卡片」和「blog 列表縮圖」，製作時照這幾點：

1. **尺寸做 1200 × 630**（約 1.91:1），這是 Facebook / LinkedIn / LINE 分享卡片的標準比例。用 4:3 之類的比例，分享時上下會被裁掉。
2. **重要內容放中間，四周各留約 80–100px 不放關鍵元素**。不同地方的裁切方式不同：

   | 出現位置                     | 呈現方式                                                    |
   | ---------------------------- | ----------------------------------------------------------- |
   | Facebook / LinkedIn 分享卡片 | 1.91:1 完整顯示                                             |
   | X（Twitter）、部分 LINE 預覽 | 可能裁成 2:1 或更窄                                         |
   | blog 列表縮圖                | 約 1.44:1，右側會淡出融入背景（`coverImageStyle = "mask"`） |

3. **檔案壓到 300KB 以下**。分享卡片抓的是你放進去的**原始檔**（不會經過網站壓縮），太大時社群平台可能抓圖逾時、卡片直接不顯示圖。
   - 插畫、截圖類：PNG，再丟 [Squoosh](https://squoosh.app) 壓縮
   - 照片類：輸出 JPG，品質 80 左右
4. **圖上有字的話，字要大**：手機上的預覽卡片通常只有約 300px 寬。放關鍵字即可，完整標題卡片本身會顯示。
5. **每篇文章用不同的圖**，不要全站共用同一張。
